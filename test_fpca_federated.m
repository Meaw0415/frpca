...
T_base = (2^max_tree_depth) * 100; % 2^(max tree depth) * 1k
% t-max
tmax = 5; % 5
...
for t = 1:tmax
  ...
  for tree_depth = 1:max_tree_depth
      ...
      chunks = 40^tree_depth;
      % set the chunk size
      chunkSize = T/chunks;
      % clear W_chunk
      clear W_chunk U_chunk G_chunk V_chunk;
...
%         V_t_1 = V_chunk1;
        ...
        U_m = NaN(chunks / 2, feats, feats);
        G_m = NaN(chunks / 2, feats, T_base*t/20);


        
        merge_tick = 0;
        for i = 1:tree_depth
          chunk_merges = chunks / (40^i);
          ...
          for w = 1:chunk_merges
            ...
            [U_m(w, :, :), G_m(w, :, :),~] = ...
              frpca_subspace_merge(squeeze(U_t(2*w-1, :, :)), ...
                                  squeeze(G_t(2*w-1, :, :)), ...
                                  squeeze(U_t(2*w, :, :)), ...
                                  squeeze(G_t(2*w, :, :)));
