...
[ul,sl,vl,us,ss,vs] = RobustPCA(dat);
% project
Uproj = U_fpca' * dat;
Uprojr = ul' * dat;
...
fig = figure;
% now plot them.
gscatter(first_pcr, second_pcr, proj_colors);
legend('hide');

% titles
tt = sprintf("F-RPCA %s (no mask)", dat_desc);
title(tt);
ylabel('2nd PC');
xlabel('1st PC');

% print the figure
st = sprintf("frpca_no_mask_r_%d_%s", rank, dat_desc);
print_fig(fig, st, params);
