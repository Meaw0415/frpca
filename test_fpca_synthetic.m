...
for trial = 1:size(alphas, 2)
  ...
  for csim = 1:nSim
    ...
    [Ufpca, Sigma_fpca, fpca_opt_out,~,~,~] = RobustPCA(Y);
...
%     rpcs_fpca(trial, csim, :) = fpca_opt_out.rpcs;
    ...
    yr_sz = size(Ufpca * Sigma_fpca*fpca_opt_out, 2);
    yr_err_sum = sum(sum((Y-Ufpca * Sigma_fpca*fpca_opt_out).^2, 1));
    fpca_fro = yr_err_sum / yr_sz;
    fpca_mse = yr_sz * immse(Y, Ufpca*Sigma_fpca*fpca_opt_out);
    ...
    if params.fpca_fixed_run == 1
      ...
      fpca_lo_params.verbose = 0;
        tic
      [Ufpca_lo, S_m, fpca_lo_opt_out] = fpca_edge(Y, lo_r, fpca_lo_params);
        toc
        temp_t = toc;
      lo_yr = size(fpca_lo_opt_out.Yr, 2);
      fro_fpca_lo = sum(sum((Y - Ufpca_lo* S_m* fpca_lo_opt_out).^2, 1)) / lo_yr;
      mse_fpca_lo = lo_yr * immse(Y, Ufpca_lo* S_m* fpca_lo_opt_out);
      fprintf(" !! Final fro: %d (low_r: %d)\n", fro_fpca_lo, lo_r);
      
      % get the time
      t_fpca_lo(csim, trial) = temp_t;
